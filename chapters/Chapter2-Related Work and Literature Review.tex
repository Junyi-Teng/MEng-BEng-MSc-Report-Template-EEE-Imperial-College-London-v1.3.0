...
the resulting numerical error.\cite{jacob2017quantizationtrainingneuralnetworks}
For large pretrained language models, avoiding full retraining makes
{PTQ} attractive when the original training data or computational
budget are unavailable.

A typical {PTQ} method uses representative calibration inputs to
...
use different calibration data, sample counts or numerical settings\cite{williams-aletras-2024-impact}.
...
accuracy of low-precision Transformer inference\cite{yao2022zeroquantefficientaffordableposttraining}.
...
each coordinate~\cite{1094577}. Classical vector quantization therefore provides a
different rate--distortion trade-off from scalar rounding. This distinction becomes important later in the chapter: \ac{GPTQ}, AWQ and SmoothQuant primarily operate with scalar or group-wise representations, whereas QuIP\# and NestQuant introduce structured vector
codebooks~\cite{tseng2024quipbetterllmquantization,savkin2025nestquantnestedlatticequantization}.


\section{Weight-Only Post-Training Quantization}

Weight-only quantization reduces the precision of the fixed model
parameters while retaining runtime activations and the {KV} cache at
...
\subsection{{GPTQ}}
GPTQ is a reconstruction-aware weight-only {PTQ} method designed for
large Transformer models\cite{frantar2023gptqaccurateposttrainingquantization}.
...
\subsection{{AWQ}}
{AWQ} is a hardware-oriented weight-only post-training quantization
...
are therefore treated as more sensitive to quantization\cite{lin2026awqactivationawareweightquantization}. This is why
...
proposed asymmetric calibration to compensate for this effect\cite{li2025gptaqefficientfinetuningfreequantization}. AWQ avoids second-order reconstruction and
...
must be considered together when comparing low-bit implementations\cite{park2024lutgemmquantizedmatrixmultiplication}.

\section{Weight--Activation Post-Training Quantization}

Weight--activation {PTQ} extends low-precision representation from
...
large-magnitude activation outliers\cite{dettmers2022llmint88bitmatrixmultiplication}. Consider a signed symmetric
...
must nevertheless be sufficiently large to represent them\cite{wei-etal-2023-outlier}.
...
fraction of the activations\cite{wei-etal-2023-outlier}~\cite{wei2023outliersuppressionpushinglimit}.

\subsection{SmoothQuant}
SmoothQuant is a training-free W8A8 {PTQ} method that addresses
...
rounding operations\cite{xiao2023icml-smoothquant}.
...
{W4A8} denotes a mixed-precision configuration in which model
...
equalisation for layers that are particularly difficult to quantize\cite{li2023fptqfinegrainedposttrainingquantization}. This work demonstrates that W4A8 generally requires
...
low-rank compensation\cite{wu2023zeroquantfpleapforwardllms}, to reduce the degradation introduced by W4A8
...
to be reconstructed through an additional low-rank path\cite{pmlr-v235-zhang24j}.
...
making it inherently more challenging than weight-only quantization\cite{shao2024omniquantomnidirectionallycalibratedquantization}.
...
LLM.int8() retains selected outlier dimensions at higher precision\cite{dettmers2022llmint88bitmatrixmultiplication}.
...
independently~\cite{xiao2023icml-smoothquant}.
...
retaining the geometric benefits of vector quantization\cite{chee2024quip2bitquantizationlarge}.
...
very-low-bit weight quantization\cite{tseng2024quipbetterllmquantization}. This work demonstrates that structured vector
...
rate--distortion problem rather than only as scalar rounding\cite{harbuzova2026pricemetricuniversalityvector}.
...
encodable reconstruction points\cite{savkin2025nestquantnestedlatticequantization}.
...
\ac{GPU}-memory measurements are reported for the dedicated packed
...
states while limiting disruption to the attention computation\cite{pmlr-v235-liu24bz}.
...
treatment of outliers\cite{hooper2025kvquant10millioncontext}. These approaches show that the KV cache is a
...
cache is actually stored and processed in that compact representation\cite{yue2024wkvquantquantizingweightkeyvalue}.
...
Other methods like rotation-based methods provide another route to controlling outliers.
QuaRot applies computationally equivalent rotations to support
low-bit quantization of weights, activations and the KV
cache~\cite{ashkboos2024quarotoutlierfree4bitinference}.
SpinQuant instead learns rotation matrices to improve quantization
accuracy~\cite{liu2025spinquantllmquantizationlearned}.

AQLM provides an alternative based on learned additive codebooks and
joint optimisation within Transformer
blocks~\cite{egiazarian2024extremecompressionlargelanguage}.
